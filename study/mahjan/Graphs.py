...
class MakeGraph:
    ...
    def plot(self, plt_lists: List[pd.DataFrame], option: List[GraphViewOptions], axis: GraphAxis,
             filter_num: int, color_list: pd.DataFrame = pd.DataFrame(data=[])) -> None:
        fig = plt.figure(figsize=(15, 25))
        fig.subplots_adjust(hspace=0.5)

        for index, list_val in enumerate(plt_lists):
            self.__ax(index=index, fig=fig, ax_list=list_val, option=option, axis=axis, filter_num=filter_num, color_list=color_list)
    ...
    def __ax(self, index: int, fig: plt.Figure, ax_list: pd.DataFrame, option: List[GraphViewOptions], axis: GraphAxis,
             filter_num: int, color_list: pd.DataFrame) -> None:
        ...
        # 色付け
        if len(color_list.index) != 0:
            self.__color_paint(color_list, ax)
    ...
    def __color_paint(self, color_list: pd.DataFrame, ax: plt.Axes) -> None:
        for index, color_list in color_list.iterrows():
            ax.axvspan(color_list['start_time'], color_list['end_time'], color=color_list['color'], alpha=0.5)
